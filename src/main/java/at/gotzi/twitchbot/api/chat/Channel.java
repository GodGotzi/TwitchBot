package at.gotzi.twitchbot.api.chat;
...
import at.gotzi.twitchbot.api.ChannelQueries;
import at.gotzi.twitchbot.api.json.JsonArray;
import at.gotzi.twitchbot.api.json.JsonObject;
import at.gotzi.twitchbot.api.json.JsonValue;
import at.gotzi.twitchbot.core.BotSequence;

public class Channel {
	
	private final String urln = "http://tmi.twitch.tv/group/user/$channel$/chatters";
	private final String channel;
	private final BotSequence bot;
	 ...
	 */
	private Channel(String channel, BotSequence bot) {
		this.bot = bot;
	 ...
	 * @return The channel
	 */
	public static Channel getChannel(String channel, BotSequence bot) {
	...
	public final List<User> getViewers() {
		...
		try {
			...
			br.close();
			JsonObject jsonObj = at.gotzi.twitchbot.api.json.JsonObject.readFrom(inputLine);
			...
			for (at.gotzi.twitchbot.api.json.JsonValue value : array)
				viewers.add(User.getUser(value.toString().substring(1, value.toString().length() - 1)));
			for (at.gotzi.twitchbot.api.json.JsonValue value : array2)
				viewers.add(User.getUser(value.toString().substring(1, value.toString().length() - 1)));
			return viewers;
		} catch (IOException e) {
			e.printStackTrace();
	...
	public final List<User> getMods() {
		...
		try {
			...
			br.close();
			at.gotzi.twitchbot.api.json.JsonObject jsonObj = at.gotzi.twitchbot.api.json.JsonObject.readFrom(inputLine);
			JsonArray array2 = jsonObj.get("chatters").asObject().get("moderators").asArray();
	...
	public final boolean isFollowing(User user) {
		try {
			...
			BufferedReader br = new BufferedReader(new InputStreamReader(conn.getInputStream()));
			at.gotzi.twitchbot.api.json.JsonObject jsonObj = at.gotzi.twitchbot.api.json.JsonObject.readFrom(br.readLine());
...
//		}
	...
	public final String getGame() {
		...
		try {
			...
			at.gotzi.twitchbot.api.json.JsonObject jsonObj = at.gotzi.twitchbot.api.json.JsonObject.readFrom(strs);
	 ...
	 * @return Returns the state
	 */
	public static final boolean isLive(Channel channel, BotSequence bot) {
		try {
			URL url = new URL("https://api.twitch.tv/kraken/streams/" + channel.toString().substring(1));
			URLConnection conn = url.openConnection();
			conn.setRequestProperty("Client-ID", bot.getClientID());
			BufferedReader br = new BufferedReader(new InputStreamReader(conn.getInputStream()));
			at.gotzi.twitchbot.api.json.JsonObject jsonObj = JsonObject.readFrom(br.readLine());
